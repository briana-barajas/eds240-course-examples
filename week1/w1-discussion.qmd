...
```{r, warning='hide', message=FALSE, warning=FALSE}
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##                                    setup                               
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
fracking_clean <- fracking %>%  
  ...
  # === clean column names ===
  janitor::clean_names() %>% #update col headers to snake case "_"
  
  # === clean up dates ===
  # remove ' AM' string from job_start_date col
  mutate(job_start_date = str_remove(job_start_date, " AM")) %>% 
  
  #convert job_start_date from string to datetime obj (save to new col)  
  mutate(datetime_start = mdy_hms(job_start_date)) %>% 
  
  #create year column using year from datetime_start column
  mutate(year = year(datetime_start)) %>% 
  
  # === select relevant columns by name ===
  select(datetime_start, year, state_name, county_name, well_name,
         total_base_water_volume) %>% 

# === clean state name column ===
# use ! to filter out non-state names
filter(!state_name %in% c("Beaver", "Beckham", "Harper", "Hemphill",
                          "Midland", "Red River", "Roosevelt", "Rusk", 
                          "State", "WARD")) %>%  
  # shorten state_name col name (optional, useful for renaming in next step
  # using case_when)
  ...
  # fix misspelled state names (mispelled ~ corrected)
  mutate(sn = case_when(
